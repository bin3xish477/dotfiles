...
PROMPT="%{$fg_bold[grey]%}[%{$reset_color%}%{$fg_bold[${host_color}]%}%n@%m%{$reset_color%}%{$fg_bold[grey]%}]%{$reset_color%} %{$fg_bold[blue]%}%10c %W %t $(ip a | grep -A 1 eth0 | grep inet | tr -s ' ' | cut -d ' ' -f 3 | sed 's/\/[0-9].*$//') %{$reset_color%} $(git_prompt_info) $(git_remote_status)
